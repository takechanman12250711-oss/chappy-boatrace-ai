...
比較データは記事監査より前に `data/outer-attack-sources/YYYYMMDD` へ不変保存する。`save-live-verification-sources.js` が原稿公開の可否と無関係にGitHubへ保存し、中央監視が検査して取り込む。原稿監査失敗時も検証は止めず、投稿ゲートは維持する。予想全体のformationsは比較元へ渡さず、実戦厳選だけをAにする。

集計内容が変わらず更新時刻だけが異なる場合はファイルを書き換えず、不要なコミットとPages再起動を避ける。中央監視のcheckoutも必要なコード・保存データだけを取得する。

展示後の全レース収集で、圧縮前の評価と実戦厳選から既存shadowを生成し、noteの不変bundleへ保存する。中央監視は保存済みshadowだけを取り込み、旧原稿へ候補を後付けしない。原稿の展示・締切前監査・買い目一致・snapshot識別を検査する。

中央監視は専用concurrencyで動き、通常予想・note収集・結果収集・candidate24結果更新後にcaptureとsettleを行う。日次原本の復元は読み取り専用とし、保存前に作業ツリーを戻す。結果は公式日次データと既存race-review-resultsの公式証拠を使用する。派生3ファイルのみを保存し、更新後にPages buildを要求する。

0件ではnoteCollectionの旧原稿未保存・評価基礎不足・信号不成立を確認する。更新が24時間以上止まったレポートを「収集中」と表示しない。予想ロジックとA/Bの採用条件は変更しない。
