...
## 2026-09-23 表紙・タイトル・購入後表示

購入済み参考写真を確認し、ユーザーが表紙と本文の両方の改善を承認。

- `formation-v3` は日付・開催場・R・公式締切をタイトルに表示。無料文はレース識別と取得済み情報の説明を中心に短縮。
- 有料冒頭は「買い目」、フォーメーション、実点数。重複する最大24点候補の一覧、内部分類名、長い展開説明、オッズ、括弧補足を掲載しない。最大24点候補の保存・成績集計は継続する。
- 連続券の集合と完全一致する範囲で圧縮する。2〜3行へ無理に揃えず、券の追加・削除や予想の変更をしない。実戦厳選と参考・万舟は別会計のまま。元の順序・オッズ・買い目は保存データと独立baselineで照合する。
- 旧bundleは公開用コピーのみ整形。詳細版・formation-v2の旧監査を維持。後からledgerが付加した参考欄も再整形する。購入見送りの表示、300円、展示、締切余裕120秒、原子的予約、最終公開照合は維持。
- オリジナル表紙 `assets/note/chappy-cover.jpg` を自動投稿へ追加。先に素材を検証し、本文入力後に画像追加→アップロード→サイズ確認の保存を行い、eyecatchの読み込みまで確認してから有料設定へ進む。失敗時に公開しない。既存Browser Use profileを使い、認証方式は変更しない。
- Workの非公開テスト下書き `na8a6aa107f51` で表紙表示と新本文の保存を確認。過去の保存原稿を用いた表示確認であり、レース記事の新形式公開成功ではない。自動投稿での表紙保存・公開は別途実行結果で確認する。


## 2026-09-23 本命・押さえ・万舟のオールレンジ掲載

ユーザーが表紙・本文とも簡素すぎると指摘し、本命・押さえ・万舟の全区分掲載を指定。この方針は前節の実戦厳選中心の表示に優先する。

- `formation-v4` は保存済み `mainSheet.tickets / coverTickets / flowTickets` と `manshuSheet.tickets` の全件を区分別に掲載。信頼度と波乱度の比較で片側を隠さず、24点への打切りもしない。区分内の同一券だけを重複除去し、元の集合と一致するフォーメーションへまとめる。
- 本命・押さえ・フォーメーション・万舟の見出し、保存された短い狙い、区分ごとの実点数を表示。実戦厳選と独立参考台帳は別枠を保つ。オッズ、括弧補足、個別内訳は戻さない。
- 新規原稿は生成済み予想のプールを渡し、旧bundleの公開コピーは `compactArticle(bundle.article, bundle.record.prediction)` で整形。予想エンジンを再実行せず、元bundleや結果・買い目は書き換えない。空の区分は保存済み買い目なしと表示する。
- 監査は各区分の本文・点数・メタデータを保存プールと独立に全件照合。実戦厳選の独立baseline、締切、300円、有料境界、購入見送り表示も維持する。万舟プールと別会計の参考台帳を混ぜない。
- 表紙を金・青・赤の明るいオリジナル画像へ差し替え、「本命・押さえ・万舟」を表示。画像アップロードの既存経路を再利用する。

## 2026-09-23 表紙コピーの修正

ユーザー指定によりブランド名「チャッピーボートレースAI」は下部へ小さく配置。主見出しを「なぜ、この艇を狙うのか。」、補助文を「展開から読む、本命・押さえ・万舟」とし、根拠を読める記事であることを伝える。金・青・赤の華やかさと既存の表紙アップロード経路を維持。的中・利益の保証や未確認の実績は加えない。
