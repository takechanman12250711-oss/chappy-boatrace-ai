...
const DEFAULTS = {
  avg:[.15,.16,.16,.16,.17,.15],
  now:[.14,.16,.16,.16,.17,.15],
  tenji:[6.90,6.92,6.88,6.85,6.91,6.93],
  lap:[37.5,37.7,37.4,37.3,37.6,37.8],
  nat:[6,5.7,5.9,6.4,6.7,6.1],
  loc:[5.8,5.4,5.9,6,6,5.9],
  home:[5.7,5.4,6,6.4,6.2,6.8],
  motor:[35,34,38,42,40,39]
};

function clamp(n){ return Math.round(Math.max(1, Math.min(99, n))); }

function nums(text, key, dec=true){
  const start = text.indexOf(key);
  if(start < 0) return [];
  const part = text.slice(start, start + 200);
  const re = dec ? /\d+\.\d+/g : /\d+/g;
  return (part.match(re) || []).map(Number).slice(0, 6);
}

function getVenue(place){ return VENUE[place] || VENUE["24"]; }

function courseValue(boat, venue){
  const values = [0, venue.in/8, venue.diff/8, venue.attack3/8, venue.keep4/8, venue.out5/8, venue.out6/8];
  return values[boat] || 0;
}

function makeRacer(boat, i, data, place, wind, water, entry, engine){
  const venue = getVenue(place);
  const r = {
    boat,
    avg: data.avg[i] ?? DEFAULTS.avg[i],
    now: data.now[i] ?? DEFAULTS.now[i],
    tenji: data.tenji[i] ?? DEFAULTS.tenji[i],
    lap: data.lap[i] ?? DEFAULTS.lap[i],
    nat: data.nat[i] ?? DEFAULTS.nat[i],
    loc: data.loc[i] ?? DEFAULTS.loc[i],
    home: data.home[i] ?? DEFAULTS.home[i],
    motor: data.motor[i] ?? DEFAULTS.motor[i]
  };

  const course = courseValue(boat, venue);
  let base = 50;
  base += (r.nat - 5.5) * 8;
  base += (r.loc - 5.5) * 7;
  base += (r.home - 5.5) * 5;
  base += (0.18 - r.avg) * 95;
  base += (0.18 - r.now) * 85;
  base += (6.95 - r.tenji) * 25;
  base += (37.8 - r.lap) * 5;
  base += (r.motor - 35) * 0.18;
  base += course;

  if(["強風","荒れ","うねり"].includes(wind) || ["荒れ","うねり"].includes(water)){
    if(boat >= 4) base += 6;
    if(boat === 1) base -= 5;
  }
  if(water === "潮あり") base += (r.loc - 5.5) * 3;
  if(entry === "move"){
    if(boat >= 4) base += 4;
    if(boat === 1) base -= 3;
  }
  ...
  if(engine === "new"){
    base -= Math.max(0, r.motor - 40) * 0.25;
    base += (0.18 - r.now) * 35;
  }

  r.akkun = clamp(base);
  r.attack = clamp(r.akkun + (boat === 3 ? 7 : 0) + (boat === 4 ? 4 : 0) + (r.now <= .13 ? 5 : 0));
  r.flow = clamp(r.akkun + course / 2);
  r.manshu = clamp(38 + (boat >= 4 ? 22 : 0) + (r.now <= .14 ? 9 : 0) + (r.tenji <= 6.88 ? 9 : 0) + (r.loc >= 6 ? 5 : 0) + venue.rough / 4 - (boat === 1 ? 18 : 0));
  r.winRate = clamp(r.akkun * 0.72 + (boat === 1 && venue.in >= 75 ? 12 : 0));
  r.manshuPct = clamp(r.manshu * 0.8);
  return r;
}

function role(boat){
  return ["","イン逃げ/残し","2コース差し・残し","3コース攻め","4コース残し/まくり差し","展開拾い穴","道中拾い/3着穴"][boat];
}

function buffs(r, engine){
  const arr = [];
  if(r.now <= .14) arr.push("⬆️今節ST +12");
  if(r.tenji <= 6.88) arr.push("⬆️展示 +8");
  if(r.lap <= 37.35) arr.push("⬆️一周 +8");
  if(r.loc >= 6) arr.push("⬆️当地勝率 +6");
  if(r.home >= 6) arr.push("⬆️地元/相性 +5");
  if(r.motor >= 40 && engine !== "new") arr.push("⬆️モーター +2");
  if(r.motor >= 40 && engine === "new") arr.push("⬇️新エンジン期：モーター過信なし");
  if(r.now >= .18) arr.push("⬇️今節ST遅め -6");
  return arr.length ? arr.join("\\n") : "大きな補正なし";
}

function missingList(){
  return ["1-4-5","1-5-4","2-1-4","2-1-5","3-1-4","3-1-5","3-4-1","3-5-1","4-1-3","4-1-5","4-3-1","4-5-1","5-1-3","5-1-4","5-3-1","5-4-1","6-1-3","6-1-4","1-6-4","1-5-6","2-4-1","2-5-1","3-4-5","3-5-4","4-2-1","5-2-1","6-2-1","4-6-1","5-6-1","6-5-1"];
}

function oddsCalc(){
  const raw = $("oddsInput").value.trim();
  if(!raw) return "未入力";
  const values = raw.split(/[,\\s、]+/).map(parseFloat).filter(x => x > 0);
  if(!values.length) return "未入力";
  return (1 / values.reduce((s, x) => s + 1/x, 0)).toFixed(1) + "倍";
}
...
function analyze(){
  ...
  const data = {
    ...
    loc: nums(text, "当地"),
    home: nums(text, "地元"),
    motor: nums(text, "モーター", false)
  };

  const racers = [1,2,3,4,5,6].map((boat, i) => makeRacer(boat, i, data, place, wind, water, entry, engine));
  render(racers, place, wind, water, entry, engine);
}

function render(racers, place, wind, water, entry, engine){
  const venue = getVenue(place);
  const rank = [...racers].sort((a,b) => b.akkun - a.akkun);
  const flowRank = [...racers].sort((a,b) => b.flow - a.flow);
  const manshuRank = [...racers].sort((a,b) => b.manshu - a.manshu);

  $("status").textContent = `反映OK：${PLACES[place]} / 風:${wind} / 水面:${water} / 進入:${entry} / エンジン:${engine}`;

  $("racers").innerHTML = racers.map(r => `
    <div class="racer">
      <b>🚤${r.boat}号艇</b>
      <div class="score">あっくん指数 ${r.akkun}点</div>
      <span class="badge">🔥攻め ${r.attack}</span>
      <span class="badge">🌊展開 ${r.flow}</span>
      <span class="badge">💣万舟 ${r.manshu}</span>
      <span class="badge">🎯本命率 ${r.winRate}%</span>
      <p>平均ST ${r.avg} / 今節ST ${r.now} / 展示 ${r.tenji} / 一周 ${r.lap} / 全国 ${r.nat} / 当地 ${r.loc} / 地元 ${r.home} / M ${r.motor}%</p>
      <pre>${buffs(r, engine)}</pre>
      <p>${role(r.boat)}</p>
    </div>`).join("");

  $("blue").textContent =
`◎本命：${flowRank[0].boat}号艇 ${flowRank[0].akkun}点
${buffs(flowRank[0], engine)}
...
艇番固定優遇なし。展開・場・水面・ST・展示・当地で根拠がある艇だけ評価。`;
  ...
  $("pink").textContent =
`万舟軸：
${manshuRank.slice(0,4).map(x=>`${x.boat}号艇 万舟${x.manshu}点 / 期待${x.manshuPct}%`).join("\\n")}
...
万舟軸：${manshuRank.slice(0,3).map(x=>x.boat+"号艇").join(" / ")}`;

  const fastest = [...racers].sort((a,b)=>a.now-b.now)[0];
  const tenjiTop = [...racers].sort((a,b)=>a.tenji-b.tenji)[0];
  const lapTop = [...racers].sort((a,b)=>a.lap-b.lap)[0];
  const sam = [...racers].sort((a,b)=>(a.tenji+a.lap)-(b.tenji+b.lap));
  const slit = [];
  for(let i=0;i<5;i++){
    const diff = Math.abs(racers[i].now - racers[i+1].now);
    if(diff >= .10) slit.push(`${racers[i].boat}-${racers[i+1].boat}間 ST差${diff.toFixed(2)} 発動`);
  }
...
一周1位 ${lapTop.boat}号艇 一周${lapTop.lap}
${tenjiTop.boat === lapTop.boat ? "同一艇で強アラート" : "別艇。展示型と道中型を分ける"}
...
万舟は${manshuRank[0].boat}号艇を軸候補に、外枠だけでなく内側絡みも見る。
特定艇の優遇は固定ではなく、場・展開・水面・ST・展示・当地が揃った時だけ。`;
}

function init(){
  $("place").innerHTML = Object.entries(PLACES).map(([k,n]) => `<option value="${k}" ${k==="24" ? "selected" : ""}>${n}</option>`).join("");
