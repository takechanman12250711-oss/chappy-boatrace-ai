...
function primaryScenarioOf(record) { const prediction = predictionOf(record); const sources = [prediction?.scenarioAiV6Shadow?.scenarios, prediction?.verificationEvidence?.scenarios, prediction?.scenarios]; for (const rows of sources) { const first = Array.isArray(rows) ? rows.find(row => row && (row.type || row.key || row.label)) : null; if (!first) continue; const scenarioKey = String(first.type || first.key || first.label || "").trim(); if (!scenarioKey) continue; return { scenarioKey, scenarioLabel: String(first.label || first.name || scenarioKey) }; } return { scenarioKey: "", scenarioLabel: "" }; }
...
function buildRows(records) {
  ...
  (Array.isArray(records) ? records : []).forEach(record => {
    ...
    const primaryScenario = primaryScenarioOf(record);
...
function summarize(rows, keyFn) {
  ...
  rows.forEach(row => {
    ...
    if (!groups.has(key)) groups.set(key, { key, label: row.label, theoryKey: row.theoryKey, jcd: row.jcd, place: row.place, scenarioKey: row.scenarioKey || "", scenarioLabel: row.scenarioLabel || "", races: new Set(), uses: 0, evaluated: 0, hits: 0, scenarioHits: 0, practicalEvaluated: 0, practicalHits: 0, skipEvaluated: 0, skipCorrect: 0, stake: 0, return: 0, ticketCount: 0, mainTicketCount: 0 });
...
function build(records) {
  ...
  return { version: "3.4.0", status: rows.length ? "collecting-data" : "no-data", sampleCount: rows.length, theoryCount: new Set(rows.map(row => row.theoryKey)).size, metricDefinitions: { practicalHitRate: "当該理論が評価可能だった終了レースにおける実戦厳選的中率", skipDecisionAccuracy: "保存済み予想から展開シャドーと見送りAIシャドーを再現し、見送りなら不的中、勝負候補なら的中を正解とした精度。注意判定は対象外", actionRanking: "評価20件未満は判断保留。20件以上で回収率100%以上かつ実戦的中率20%以上を強化候補、回収率70%以上または実戦的中率20%以上を維持、それ以外を弱化候補として監視。自動適用はしない。" }, byTheory, theoryActionRanking: actionRanking(byTheory), byVenueTheory: summarize(rows, row => `${row.jcd}:${row.theoryKey}`), byVenueScenarioTheory: summarize(rows.filter(row => row.evaluated && row.scenarioKey), row => `${row.jcd}:${row.scenarioKey}:${row.theoryKey}`), usableForPrediction: false, automaticApplication: false };
}

module.exports = { pct, normalizeTicket, evaluationRows, predictionOf, practicalHitOf, confidenceOf, completenessOf, scenarioShadowOf, primaryScenarioOf, skipDecisionOf, skipDecisionCorrect, buildRows, summarize, actionOf, actionRanking, build };
