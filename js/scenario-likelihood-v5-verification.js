...
function actualScenario(result) {
  ...
  if (winner === 1 && /逃げ/.test(method)) {
    return { key: "inEscape", label: "1逃げ" };
  }
  if (winner === 2 && /差し/.test(method)) {
    return { key: "course2Sashi", label: "2差し" };
  }
  if (winner === 3 && /まくり|まくり差し|差し/.test(method)) {
    return { key: "course3Attack", label: "3攻め" };
  }
  if (winner === 4 && /まくり|まくり差し|差し/.test(method)) {
    return { key: "course4Kado", label: "4カド" };
  }
...
function verify(shadow, result) {
  ...
  const scenarios = Array.isArray(shadow?.scenarios)
    ? shadow.scenarios
    : [];
...
function buildSummary(rows) {
  ...
  Object.values(byAmbiguity).forEach(bucket => {
    bucket.leaderHitRate = bucket.count
      ? Math.round(bucket.leaderHits / bucket.count * 1000) / 10
      : 0;
    bucket.top2HitRate = bucket.count
      ? Math.round(bucket.top2Hits / bucket.count * 1000) / 10
      : 0;
  });
  ...
  return {
    ...
    leaderHits,
    leaderHitRate: comparable.length
      ? Math.round(leaderHits / comparable.length * 1000) / 10
      : 0,
    top2Hits,
    top2HitRate: comparable.length
      ? Math.round(top2Hits / comparable.length * 1000) / 10
      : 0,
    byAmbiguity
  };
}
