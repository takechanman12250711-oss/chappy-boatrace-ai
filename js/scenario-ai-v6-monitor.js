...
function build(rollout = {}, abReport = {}, options = {}) {
  ...
  const harmfulVenueCount = (Array.isArray(abReport?.byVenue) ? abReport.byVenue : [])
    .filter(row => Number(row?.comparableCount || 0) >= 10 && Number(row?.aWins || 0) - Number(row?.bWins || 0) >= 4)
    .length;
