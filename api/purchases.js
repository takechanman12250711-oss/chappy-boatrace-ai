...
async function triggerSettlement(req, env = process.env, fetchImpl = fetch) {
  ...
  if (!origin || !token) return { triggered: false, reason: "settlement_endpoint_unavailable" };
  try {
    ...
    return { triggered: false, reason: String(error?.message || error) };
  }
}
...
function parseBody(req) {
  ...
  try { return JSON.parse(req.body); } catch { throw new Error("JSON形式が不正です"); }
}

function normalizeBatch(body) {
  const source = Array.isArray(body) ? body : Array.isArray(body?.purchases) ? body.purchases : [];
  if (!source.length) throw new Error("purchasesが空です");
  if (source.length > MAX_BATCH_SIZE) throw new Error(`1回の同期は${MAX_BATCH_SIZE}件までです`);
  const normalized = source.map(purchaseSync.normalizePurchase).filter(Boolean);
  if (!normalized.length) throw new Error("有効な購入明細がありません");
  if (normalized.length !== source.length) throw new Error("不正な購入明細が含まれています");
  return normalized;
}
...
function filterPurchases(purchases, query = {}) {
  ...
  return purchases.filter((purchase) => (!date || purchase.date === date) && (!raceKey || purchase.raceKey === raceKey));
}
...
async function handler(req, res, dependencies = {}) {
  ...
  if (!isAuthorized(req, env)) return res.status(401).json({ ok: false, error: "購入同期の認証に失敗しました" });

  try {
    if (req.method === "GET") {
      const purchases = await purchaseStore.loadPurchases({ env });
      const filtered = filterPurchases(purchases, req.query || {});
      return res.status(200).json({ ok: true, count: filtered.length, purchases: filtered });
    }
    ...
    return res.status(405).json({ ok: false, error: "GETまたはPOSTを使用してください" });
  } catch (error) {
...
module.exports = function purchasesHandler(req, res) { return handler(req, res); };
module.exports._test = { MAX_BATCH_SIZE, getBearerToken, isAuthorized, getOrigin, triggerSettlement, parseBody, normalizeBatch, filterPurchases, handler };
