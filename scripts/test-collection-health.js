    ...
    },
    {
      ...
      collectionHealth: {
        ...
        targets: [
          ...
          { raceKey: "20260722-23-2", jcd: "23", place: "唐津", status: "invalid_boat_identity", missingReasons: ["艇番不整合"] },
...
assert.equal(report.monitoredCount, 5);
assert.equal(report.savedCount, 2);
assert.equal(report.missingCount, 3);
assert.equal(report.insufficientDataCount, 1);
assert.equal(report.invalidBoatIdentityCount, 1);
assert.equal(report.failedCount, 1);
assert.equal(report.coverageRate, 40);
...
assert.equal(report.venues.find(item => item.jcd === "23").invalidBoatIdentityCount, 1);
