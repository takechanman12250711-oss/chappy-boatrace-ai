...
const owner = fs.readFileSync("js/final-display-controller.js", "utf8");
const ownerCss = fs.readFileSync("css/final-display-controller.css", "utf8");

assert(loader.includes('OWNER_BUILD="20260905-final-display-owner1"'), "final display owner cache key missing");
assert(loader.includes("final-display-controller.js"), "single final display controller is not loaded");
assert(loader.includes("final-display-controller.css"), "single final display controller stylesheet is not loaded");
...
assert(owner.includes("buildFlowFormations"), "formation ownership missing");
...
assert(owner.includes("本命・押さえ等の説明は繰り返さず"), "practical duplicate suppression missing");
