...
const START_DATE = "20260819";
const ADOPTION_PR = 462;
const ADOPTION_MERGED_AT = "2026-08-18T15:01:06Z";
...
const outputPath = path.join(
  ...
  "data",
  "stats",
  "candidate90-course2-prune-post-adoption-monitor.json"
);

function loadSelectorBeforeCourse2Prune() {
  ...
  if (!original.includes(marker)) {
    throw new Error("approved course2 candidate90 prune marker not found");
  }
  ...
  if (patched === original) {
    throw new Error("course2 counterfactual patch was not applied");
  }
...
const selectorBeforeCourse2Prune = loadSelectorBeforeCourse2Prune();
...
function isPostAdoption(row) {
  const capturedAt = capturedAtOf(row);
...
function build() {
  ...
  for (const filename of files) {
    ...
    for (const row of rowsOf(data)) {
      ...
      const productionPrediction = global.createPrediction(input);
      const counterfactualPrediction = global.createPrediction(input);
      const production = productionSelector.select(productionPrediction);
      const counterfactual = selectorBeforeCourse2Prune.select(counterfactualPrediction);
      ...
      if (changed) {
        const hasApprovedPruneAudit = (production.candidateDecisions || []).some(
          decision => decision?.reasonCode === COURSE2_PRUNE_REASON
        );
        if (!hasApprovedPruneAudit) {
          throw new Error(`${raceKey}: selection changed without approved course2 candidate90 prune audit`);
        }
      }
  ...
  const report = {
    schemaVersion: 1,
    version: "candidate90-course2-prune-post-adoption-monitor-v1",
    generatedAt: new Date().toISOString(),
    adoptionPr: ADOPTION_PR,
    adoptionMergedAt: ADOPTION_MERGED_AT,
    startDate: START_DATE,
    latestDate,
    method: "replay only rows captured at/after PR #462 merge; A=current production selector, B=in-memory counterfactual reverting only the actual-course2 candidate90 prune while rerunning all downstream selection normally",
    productionChanged: false,
    automaticApplication: false,
    interpretation: "Positive hit/profit delta means the approved PR #462 production rule outperformed the same current-main replay with only that rule reverted.",
...
module.exports = {
  ...
  finalize,
  isPostAdoption,
  loadSelectorBeforeCourse2Prune
};
