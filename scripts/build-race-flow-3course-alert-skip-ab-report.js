...
const output = path.join(root, "data", "stats", "race-flow-3course-alert-skip-ab-report.json");
const STAKE_PER_TICKET = 100;
const TARGET_LABEL = "3コース攻め警戒";
const PROSPECTIVE_CUTOFF = "2026-08-17T06:55:00Z";
...
function raceKey(record = {}) { return `${String(record.date || "")}-${String(record.jcd || "").padStart(2, "0")}-${Number(record.raceNo || 0)}`; }
function load(dir) { if (!fs.existsSync(dir)) return []; return fs.readdirSync(dir).filter(name => /^\d{8}\.json$/.test(name)).sort().map(name => JSON.parse(fs.readFileSync(path.join(dir, name), "utf8"))); }
...
function settle(rows, mode) {
  let settledCount = 0, betRaceCount = 0, skippedRaceCount = 0, hitCount = 0, stake = 0, returned = 0;
  for (const row of rows) {
    if (!row.result) continue;
    settledCount++;
    const skip = mode === "B" && scenarioLabel(row.record) === TARGET_LABEL;
    if (skip) { skippedRaceCount++; continue; }
    const ts = practicalTickets(row.record); if (!ts.length) continue;
    betRaceCount++; stake += ts.length * STAKE_PER_TICKET;
    const actual = ticket(row.result?.trifecta?.combination);
    const payout = Math.max(0, Number(row.result?.trifecta?.payout || 0));
    if (actual && ts.includes(actual)) { hitCount++; returned += payout; }
  }
...
function build(predDocs, resultDocs) {
  ...
  return {
    schemaVersion: 1,
    version: "race-flow-3course-alert-skip-ab-v1-prospective",
    generatedAt: new Date().toISOString(),
    productionChanged: false,
    targetLabel: TARGET_LABEL,
    prospectiveProtocol: { cutoffSelectedAtInclusive: PROSPECTIVE_CUTOFF, oldRecordsBackfilled: false, retrospectiveClassificationAllowed: false, actualPurchase: false },
    cohort: { raceCount: cohort.length, settledCount: cohort.filter(row => row.result).length, targetRaceCount: targetRows.length, targetSettledCount: targetRows.filter(row => row.result).length },
    a: { label: "current-A", rule: "現行どおり全対象を購入候補として評価", ...a },
    b: { label: "skip-3course-alert-B", rule: `展開ラベルが「${TARGET_LABEL}」の時だけ購入見送り`, ...b },
    delta: { stake: b.stake - a.stake, return: b.return - a.return, profit: b.profit - a.profit, recoveryRate: a.recoveryRate !== null && b.recoveryRate !== null ? Math.round((b.recoveryRate - a.recoveryRate) * 10) / 10 : null, hitRate: a.hitRate !== null && b.hitRate !== null ? Math.round((b.hitRate - a.hitRate) * 10) / 10 : null },
    interpretation: { minimumTargetSettledCount: 30, automaticApplication: false, usableForPrediction: false, affectsCurrentTickets: false, adoptionDecisionReady: targetRows.filter(row => row.result).length >= 30 }
  };
}
function main() {
  ...
  console.log(`3コース攻め警戒 prospective A/B: cohort ${report.cohort.raceCount}R / target settled ${report.cohort.targetSettledCount}R`);
}
