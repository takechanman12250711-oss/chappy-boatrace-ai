...
function collect(){if(!fs.existsSync(predictionsDir))return[];const rows=[];fs.readdirSync(predictionsDir).filter(n=>/^\d{8}\.json$/.test(n)).sort().forEach(n=>{const d=load(path.join(predictionsDir,n),{});rows.push(...(Array.isArray(d.predictions)?d.predictions:[]),...(Array.isArray(d.verificationPredictions)?d.verificationPredictions:[]));});return rows;}
