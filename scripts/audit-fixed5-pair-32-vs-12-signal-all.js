...
const P={discovery:{keep12:[],shift32:[]},holdout:{keep12:[],shift32:[]}},seen=new Set();
for(const fn of fs.readdirSync(DIR).filter(x=>/^\d{8}\.json$/.test(x)).sort()){const date=fn.slice(0,8),d=JSON.parse(fs.readFileSync(path.join(DIR,fn),"utf8"));for(const r of rows(d)){if(r?.result?.settled!==true)continue;const rk=r.raceKey||`${date}-${r.jcd}-${r.raceNo}`;if(seen.has(rk))continue;seen.add(rk);try{const i=input(r),actual=ticket(r?.result?.resultTicket||r?.result?.review?.resultTicket);if(!i||!actual)continue;const p=core.buildPredictionData(i),base=fixed(p);if(base.length!==5)continue;const has12=base.some(t=>t.startsWith("1-2-")),has32=base.some(t=>t.startsWith("3-2-"));if(!has12||has32)continue;const ap=actual.split("-").slice(0,2).join("-");if(ap!=="1-2"&&ap!=="3-2")continue;const an=analyses(p),b1=find(an,1),b3=find(an,3);if(!b1||!b3)continue;const rec={};for(const k of KEYS)rec[k]=metric(b3,k)-metric(b1,k);P[date<H?"discovery":"holdout"][ap==="3-2"?"shift32":"keep12"].push(rec);}catch{}}}
...
const out={schemaVersion:1,holdoutStart:H,target:"current fixed5 has 1-2 and no 3-2; actual pair 1-2 vs 3-2",periods:{},signals:{}};
for(const per of ["discovery","holdout"]){const a=summarize(P[per].keep12),b=summarize(P[per].shift32);out.periods[per]={keep12:a,shift32:b};for(const k of KEYS){out.signals[k]=out.signals[k]||{};out.signals[k][per]=b[k]-a[k];}}
...
out.notes={productionChanged:false,oddsUsed:false,allPairPatterns:true,currentFixed5IncludesPair31Rescue:true,goal:"identify reproducible pre-race signals for retaining 1-2 vs reallocating one 1-2 slot to 3-2"};
