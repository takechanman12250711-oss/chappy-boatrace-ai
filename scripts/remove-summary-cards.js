...
for (const heading of headings) text = removeBlockByHeading(text, heading);
for (const heading of headings) {
  if (text.includes(`<h3>${heading}</h3>`)) throw new Error(`削除に失敗しました: ${heading}`);
}
fs.writeFileSync(path, text);
console.log("summary cards removed");
