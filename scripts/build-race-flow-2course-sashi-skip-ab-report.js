...
const output = path.join(root, "data", "stats", "race-flow-2course-sashi-skip-ab-report.json");
const STAKE_PER_TICKET = 100;
const TARGET_LABEL = "2コース差し";
const PROSPECTIVE_CUTOFF = "2026-08-17T07:06:00Z";

function ticket(value) { const s = String(value?.ticket || value || "").trim(); return /^[1-6]-[1-6]-[1-6]$/.test(s) && new Set(s.split("-")).size === 3 ? s : ""; }
...
function build(predDocs, resultDocs) {
  ...
  return {
    ...
    b: { label: "skip-2course-sashi-B", rule: `展開ラベルが「${TARGET_LABEL}」の時だけ購入見送り`, ...b },
