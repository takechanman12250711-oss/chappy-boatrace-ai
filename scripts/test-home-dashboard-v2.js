...
assert(js.includes("おすすめレース"), "おすすめをレース画面へ表示する");
...
assert(referenceCss.includes("repeat(3"), "下部ナビにレース・AI予想・成績分析を表示する");
assert(!html.includes('data-view="home"'), "役割のないホームタブを表示しない");
...
assert(shellSource.includes("home-v2-recommend"), "レース画面はおすすめを表示できる");
assert(!/home-v2-filter-shell|home-v2-schedule|data-home-venues/.test(shellSource), "おすすめ枠に開催場選択を重複生成しない");
assert(js.includes('setView("race")'), "起動直後はレース画面を表示する");
assert(js.includes("raceSection.insertBefore"), "おすすめをレース画面内へ配置する");
...
assert(/const changed\s*=\s*state\.currentView !==\s*normalizedView/.test(js), "同じ画面の再タップで重い更新イベントを再送しない");
