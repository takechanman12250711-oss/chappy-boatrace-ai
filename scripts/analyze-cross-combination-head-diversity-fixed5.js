...
function partsOf(ticket) {
  return ticketOf(ticket).split("-").map(Number);
}
...
function headSet(tickets) {
  return [...countPosition(tickets, 0).keys()].sort((a, b) => a - b).join(",");
}
...
function protectedRescueIndex(prediction, current) {
...
function findHeadDiversitySafeReplacement(prediction, current) {
  const headCounts = countPosition(current, 0);
  const rescueIndex = protectedRescueIndex(prediction, current);

  // 本線3点は固定。押さえは低優先の5点目から確認する。
  for (const index of [4, 3]) {
    if (index === rescueIndex) continue;
    const removedHead = partsOf(current[index])[0];
    if ((headCounts.get(removedHead) || 0) <= 1) continue;
...
function buildNext(prediction, current) {
  ...
  const replacement = findHeadDiversitySafeReplacement(prediction, current);
  if (!replacement) {
    return { applied: false, reason: "all-safety-heads-unique-or-protected", next: current };
  }
  ...
  const beforeHeads = headSet(current);
  const afterHeads = headSet(next);
  if (beforeHeads !== afterHeads) {
    throw new Error(`head diversity changed: ${beforeHeads} -> ${afterHeads}`);
  }
...
function summarize(rows) {
  ...
  for (const row of rows) {
    ...
    if (row.applied) {
      selected += 1;
      incrementObject(removedHeads, String(row.replacement.removedHead));
      incrementObject(addedHeads, String(partsOf(row.candidate.ticket)[0]));
    }
  ...
  for (const filename of fs
    ...
    for (const row of rowsOf(doc)) {
      ...
      try {
        ...
        incrementObject(skipReasons, result.reason);
        if (result.applied && result.beforeHeads !== result.afterHeads) {
          headSetViolations += 1;
        }
        ...
        records.push({
          ...
          beforeHeads: result.beforeHeads || headSet(current),
          afterHeads: result.afterHeads || headSet(current),
          combinationMiss
        });
      } catch (error) {
    ...
    JSON.stringify(
      {
        ...
        rule: {
          ...
          replacement: "only safety index 4 then 3; main three and production third-six rescue protected",
          headDiversity: "a ticket may be removed only when another ticket with the same head remains",
        ...
        gatePassed,
        gate:
          "discovery and holdout each require selected>=20, hitDelta>0, gains>=losses, returnDelta>=0, gains on >=3 dates and >=3 venues; failures=0; headSetViolations=0; stakeDelta=0",
        headSetViolations,
        failures,
        notes: {
          ...
          allExistingHeadCandidatesPreserved: true,
