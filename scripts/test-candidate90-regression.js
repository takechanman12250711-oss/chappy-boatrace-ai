...
const expected={n:341,baseHits:106,newHits:115,added:250,trainBase:45,trainNew:49,testBase:61,testNew:66,incrementalReturn:26520};
for(const [k,v] of Object.entries(expected)){if(stats[k]!==v)throw new Error(`${k}: expected ${v}, got ${stats[k]}`);}
const recovery=Number((stats.incrementalReturn/(stats.added*100)*100).toFixed(2));if(recovery!==106.08)throw new Error(`incremental recovery expected 106.08, got ${recovery}`);
