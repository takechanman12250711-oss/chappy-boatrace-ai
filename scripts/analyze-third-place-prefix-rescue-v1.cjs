...
function empty(name) {
  return {name,raceCount:0,addedTickets:0,rescueCount:0,manboatRescueCount:0,investmentYen:0,returnYen:0,profitYen:0,roiPercent:0,baselineHitCount:0,totalHitCount:0,totalHitRatePercent:0,thirdMissRescueCount:0,missingPayoutRescueCount:0};
}
...
function evaluate(records,maxAdd,payouts) {
  ...
  for (const r of records) {
    ...
    const raceKey=r.__analysisRaceKey||input.raceKey(r);
    ...
    if(!baseHit&&hitAdded){
      out.rescueCount++;
      if(payout>0){out.returnYen+=payout; if(payout>=10000) out.manboatRescueCount++;}
      else out.missingPayoutRescueCount++;
