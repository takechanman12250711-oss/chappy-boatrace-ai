...
function record(i, slit, wallState, stFormal) {
  return {
    date: "20260817",
    jcd: "05",
    raceNo: i,
    selection: { scenarioLabel: "3コース攻め" },
    prediction: {
...
function result(r, hit, payout = 1000) {
  return {
    date: r.date,
    jcd: r.jcd,
    raceNo: r.raceNo,
...
const results = [];
for (let i = 1; i <= 12; i++) {
  const r = record(i, -3, "壁崩れ", false);
  rows.push(r);
  results.push(result(r, i === 1, 500));
}
for (let i = 13; i <= 24; i++) {
  const r = record(i, 5, "壁成立", true);
  rows.push(r);
  results.push(result(r, true, 900));
}
...
assert.ok(
  report.summaries.slit_negative.recoveryRate <
  report.summaries.slit_positive.recoveryRate
);
assert.ok(
