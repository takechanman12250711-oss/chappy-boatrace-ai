...
const prediction = {
  ...
  practicalTickets: [{
    ...
    category: "本線",
    categories: ["本線"],
    roleClaims: [{ role: "attack", boatNo: 2, expectedPositions: [1] }]
  }],
  verificationEvidence: {
    ...
    mainScenario: { label: "2差し本線", headBoatNo: 2, expectedWinningMethods: ["差し"] },
    roleClaims: [{ role: "attack", boatNo: 2, expectedPositions: [1] }],
    theoryClaims: [{
      theoryKey: "flow",
      label: "展開",
...
const officialResult = {
  ...
  winningMethod: "差し",
  trifecta: { combination: "2-1-3", payout: 1250, popularity: 4 }
};

const before = verification.verifyPrediction(JSON.parse(JSON.stringify(prediction)), officialResult);
const index = {
  predictions: [],
...
assert.deepEqual(compacted.practicalTickets[0].theoryClaims, prediction.verificationEvidence.tickets[0].theoryClaims);
[
  "scenarioMatched",
  "practicalHit",
  "missType"
...
assert.deepEqual(after.ticketCategoryResults, before.ticketCategoryResults, "券種別評価を維持する");
assert.deepEqual(
...
assert.equal(index.runs[0].collectionHealth.targets, undefined, "indexでは未使用のrun対象明細を削除する");
...
assert.equal(mismatch.verificationPredictions[0].prediction.isRetrospective, false, "値が異なる場合は削除しない");
