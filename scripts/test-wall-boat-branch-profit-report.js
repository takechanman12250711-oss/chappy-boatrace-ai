...
function rec(i,state,score,grade="A",selectedAt=`2026-08-17T04:${String(i%60).padStart(2,"0")}:00Z`){return{date:"20260817",jcd:"12",raceNo:i,selectedAt,prediction:{practicalTickets:["3-1-2"],aiCore:{wallTheory:{attackerNo:3,wallCandidateNo:2,wallBoat:state==="壁成立"?2:0,state,score,grade}}}};}
...
const preds=[],results=[];
const old=rec(99,"壁成立",70,"B","2026-08-17T03:30:00Z");preds.push(old);results.push(result(old,true,700));
...
assert.equal(report.prospectiveProtocol.oldRecordsBackfilled,false);
assert.equal(report.diagnostics.selected.formalWallEvidenceRaceCount,37);
assert.equal(report.diagnostics.selected.prospectiveFormalWallEvidenceRaceCount,36);
assert.equal(report.diagnostics.prospectiveDeduplicatedFormalRaceCount,36);
