...
async function loginNoteViaX(page, { env = process.env } = {}) {
  ...
  if (!credentials.configured) return { ok: false, reason: 'x_credentials_missing' };
  ...
  if (!passwordVisible) return { ok: false, reason: 'x_password_field_missing_or_challenge', finalUrl: page.url() };
