...
function arrays(root) {
  ...
  function walk(v) {
    ...
    if (Array.isArray(v.verificationPredictions)) for (const x of v.verificationPredictions) out.push(x);
...
function conditionOf(r) {
  const prediction = r?.prediction || r || {};
  return prediction?.preRaceConditions || r?.preRaceConditions || null;
}
function timeOf(r) {
  const vals = [
    r?.selectedAt,
...
function load() {
  ...
  for (const f of files) {
    ...
    for (const r of arrays(root)) {
      ...
      const key=String(r?.raceKey || r?.prediction?.raceKey || `${f}:${t ?? 'na'}`);
      if (seenKeys.has(key)) continue;
      if (t !== null && t >= ACTIVATED_AT && c?.sourceTiming === 'pre_deadline' && c?.officialResultUsed === false) {
        seenKeys.add(key);
        rows.push({file:f,time:t,condition:c});
      }
    }
  }
  return rows;
...
function build(){
  ...
  return {schemaVersion:2,auditId:'prerace-theory-signal-forward-v1',generatedAt:new Date().toISOString(),activatedAt:new Date(ACTIVATED_AT).toISOString(),productionChanged:false,automaticApplication:false,usableForPrediction:false,recordContract:{condition:'prediction.preRaceConditions or record.preRaceConditions',captureTime:'selectedAt/capturedAt/createdAt',deduplication:'raceKey'},eligibleSnapshotCount:rows.length,availability:{attack,flow,total,fullAttackSnapshots:full(rows,'attackStrength'),fullFlowSnapshots:full(rows,'raceFlowPower'),fullTotalSnapshots:full(rows,'totalScore'),completeAllSnapshots:completeAll,completeAllRate:rows.length?Number((100*completeAll/rows.length).toFixed(1)):0},gates:GATES.map(n=>({n,reached:rows.length>=n,status:rows.length>=n?'ready_for_signal_outcome_analysis':'collecting'})),policy:'Observation only. No score, ticket, note, purchase, or UI change.'};
}
if(require.main===module) process.stdout.write(JSON.stringify(build(),null,2)+'\n');
module.exports={build,conditionOf,timeOf};
