...
const output = path.join(root, "data", "stats", "race-flow-in-first-outside-alert-skip-ab-report.json");
const STAKE_PER_TICKET = 100;
const TARGET_LABEL = "イン先マイ・外攻め警戒";
const PROSPECTIVE_CUTOFF = "2026-08-17T07:18:00Z";
...
function build(predDocs, resultDocs) {
  ...
  const rows = [...selected, ...verification]
    .filter(isProspective)
    .map(record => ({ record, result: embeddedResult(record) || results.get(raceKey(record)) || null }));

  const dedup = new Map();
  for (const row of rows) {
    const key = raceKey(row.record);
    if (!dedup.has(key) || selectedKeys.has(key)) dedup.set(key, row);
  }
  ...
  return {
    schemaVersion: 1,
    version: "race-flow-in-first-outside-alert-skip-ab-v1-prospective",
    ...
    b: { label: "skip-in-first-outside-alert-B", rule: `展開ラベルが「${TARGET_LABEL}」の時だけ購入見送り`, ...b },
    delta: {
