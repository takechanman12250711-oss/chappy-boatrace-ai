...
function auditAdditionalTickets(paidText, prediction, issue) {
  ...
  normalized(paidText).split("\n").forEach(line => {
    ...
    } else if (/^【|^🔥 実戦厳選|^買い目(?:候補)?$|^[🔵🟠🔷🌸] /u.test(text)) {
...
// Independently compare each visible section with its original saved pool.
function auditAllRanges(article, prediction, issue) {
  const groups = [
    ["main", "🔵 本命", prediction?.mainSheet?.tickets],
    ["cover", "🟠 押さえ", prediction?.mainSheet?.coverTickets],
    ["flow", "🔷 フォーメーション", prediction?.mainSheet?.flowTickets],
    ["hole", "🌸 万舟", prediction?.manshuSheet?.tickets]
  ];
  for (const [key, heading, rows] of groups) {
    const expected = [...new Set(ticketsOf(rows))];
    const label = heading.slice(3);
    const matches = [...normalized(article.paidText).matchAll(new RegExp(
      `^${heading}\\n([\\s\\S]*?)\\n${label} (\\d+)点$`, "gm"))];
    const snapshot = article.allRangeGroups?.filter(group => group.key === key);
    if (expected.some(ticket => !validTicket(ticket)) || snapshot?.length !== 1 ||
        !sameList(snapshot[0]?.tickets, expected) || matches.length !== 1) {
      issue("ALL_RANGE_SOURCE_MISMATCH", `${label}の保存済み買い目・区分を確認できません。`);
      continue;
    }
    const lines = matches[0][1].split("\n").filter(line => line.startsWith("・"));
    const rendered = lines.flatMap(line => ticketMentions(line).flatMap(expandedMention));
    if (lines.some(line => !/^・[1-6]+-[1-6]+-[1-6]+$/.test(line)) ||
        !sameList([...rendered].sort(), [...expected].sort()) || Number(matches[0][2]) !== expected.length) {
      issue("ALL_RANGE_TICKETS_MISMATCH", `${label}の全買い目・点数が保存予想と一致しません。`);
    }
  }
}
  ...
  try {
    ...
    if (!Number.isFinite(deadline)) {
      ...
      if (jst.slice(0, 10).replaceAll("-", "") !== date || display !== clock ||
          ...
          (["formation-v3", "formation-v4"].includes(article.format) && !normalized(article.title).includes(`締切 ${clock}`))) {
    ...
    if (article.format === "formation-v4") auditAllRanges(article, record.prediction, issue);
    const practicalBlocks = [...normalized(paidText).matchAll(
      article.format === "formation-v4"
        ? /^🔥 実戦厳選\n([\s\S]*?)\n計\s+(\d+)点$/gm
    ...
    if (practicalBlocks.length !== 1) {
      ...
      const formations = ["formation-v2", "formation-v3", "formation-v4"].includes(article.format);
    ...
    const boats = ["concise-v1", "formation-v2", "formation-v3", "formation-v4"].includes(article.format)
