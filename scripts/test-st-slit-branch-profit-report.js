...
function roles(adj,formal=true){return Array.from({length:6},(_,index)=>({boatNo:index+1,status:index===2?(adj>0?"advantage":adj<0?"risk":"neutral"):"neutral",isFormal:formal,appliedToScore:formal}));}
function rec(i,adj,opts={}){const stRoles=roles(adj,opts.formal!==false);return {date:"20260816",jcd:"12",raceNo:i,prediction:{practicalTickets:["1-2-3"],aiCore:{stSlitTheory:{roles:stRoles},analyses:stRoles.map(row=>({boatNo:row.boatNo,fCount:row.boatNo===3&&opts.f?1:0,stTheory:row})),raceScenarios:{scenarios:[{type:"threeAttack",slitAdjustment:adj}],evidence:{slit:{alerts:opts.alert?[{boatNo:3}]:[],risks:opts.risk?[{boatNo:4}]:[],advantages:opts.advantage?[{boatNo:3}]:[]}}}}}};}
function storedRec(i,adj){return {date:"20260816",jcd:"12",raceNo:i,selectedAt:`2026-08-16T13:${String(i%60).padStart(2,"0")}:00Z`,prediction:{practicalTickets:["1-2-3"],verificationEvidence:{scenarios:[{type:"threeAttack",attacker:3,slitAdjustment:adj,slitReasons:[adj>0?"3号艇の展示STが優勢":adj<0?"3号艇の展示STが劣勢":"ST補正なし"]}],stSlit:{roles:roles(adj)}}}};}
...
const commentOnly=storedRec(27,0);
commentOnly.prediction.verificationEvidence.stSlit.roles=[{boatNo:3,status:"neutral",isFormal:true,appliedToScore:false}];
assert.equal(engine.stEvidence(commentOnly).formal,false,"1艇だけの表示用ST役割を正式スコア証拠にしない");
...
assert.equal(report.evidenceDiagnostics.formalScoreEvidenceRaceCountAll,26);
...
assert.deepEqual(report.evidenceDiagnostics.verificationPredictions,{raceCount:3,scenarioEvidenceRaceCount:3,adjustmentEvidenceRaceCount:3,stSlitRoleEvidenceRaceCount:3,formalScoreEvidenceRaceCount:3});
