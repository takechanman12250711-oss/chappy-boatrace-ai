...
締切表示は保存レコードから渡し、選定後の公式オッズを予想・独立baselineへ対応付ける。買い目や順序を変えて監査を通さない。取得出典と日時は新規bundleへ保存する。合格bundleは分析前にmainへ保存し、既存投稿workflowをdispatchする。投稿側は最新bundleを読み取り専用で集約するため、古いhandoffの更新を待たない。途中の保存・dispatch失敗時は公開せず、予想保存を終えてからworkflow失敗を報告する。詳細は最新の `docs/note-automation.md` を参照する。
